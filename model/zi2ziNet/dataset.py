...
def get_batch_iter(examples, batch_size, augment, shuffle_pair=False):
    ...
    def batch_iter():
        if not shuffle_pair:
            ...
            import random
            padded_clone = padded[:]
            random.shuffle(padded_clone)
            for i in range(0, len(padded), batch_size):
                batch_pair = list(zip(padded[i: i + batch_size], padded_clone[i:i + batch_size]))
